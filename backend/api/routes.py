...
# Only one simulation may run at a time — prevents double-invocation from
# React Strict Mode or accidental double-clicks swamping the rate limiter.
...
def simulate(req: RunSimulationRequest):
    if not _sim_lock.acquire(blocking=False):
        raise HTTPException(status_code=429, detail="A simulation is already running. Please wait for it to finish.")
    try:
